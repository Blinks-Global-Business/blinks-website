...
export default function BlogGrid({ posts }) {
  ...
  const formatDate = (dateStr) =>
    new Date(dateStr).toLocaleDateString(dateLocale, { day: "2-digit", month: "short", year: "numeric" });

  const featured = posts.find((p) => p.featured);
  const rest = posts.filter((p) => !p.featured);

  const filtered = activeCategory === "tous" ? rest : rest.filter((p) => p.category === activeCategory);
  ...
  return (
    <div>
      ...
      {featured && activeCategory === "tous" && (
        ...
        >
          <BlogImage src={featured.image} alt={featured.title[locale]} className="w-full h-56 md:h-full" />
          <div className="p-8 flex flex-col justify-center">
            <span className="inline-block bg-primary/10 text-primary text-[11px] font-body font-medium uppercase tracking-wide px-2.5 py-1 rounded mb-4 w-fit">
              {BLOG_CATEGORIES.find((c) => c.id === featured.category)?.label[locale]}
            </span>
            <h2 className="font-heading font-bold text-xl md:text-2xl text-text mb-3">{featured.title[locale]}</h2>
            <p className="font-body text-sm text-text-muted mb-5">{featured.excerpt[locale]}</p>
            <div className="flex items-center gap-2 font-body text-xs text-text-muted">
              ...
              <span>·</span>
              <span>{featured.readTime[locale]}</span>
            </div>
          </div>
        </Link>
      )}
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {visible.map((post) => (
          ...
          >
            <BlogImage src={post.image} alt={post.title[locale]} className="w-full h-44" />
            <div className="p-5">
              <span className="inline-block bg-primary/10 text-primary text-[11px] font-body font-medium uppercase tracking-wide px-2.5 py-1 rounded mb-3">
                {BLOG_CATEGORIES.find((c) => c.id === post.category)?.label[locale]}
              </span>
              <h3 className="font-heading font-semibold text-base text-text mb-2 line-clamp-2">{post.title[locale]}</h3>
              <p className="font-body text-sm text-text-muted mb-4 line-clamp-1">{post.excerpt[locale]}</p>
              <div className="flex items-center gap-2 font-body text-xs text-text-muted">
